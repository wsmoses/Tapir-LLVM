...
struct XformCbitStores : public ModulePass {
  ...
  XformCbitStores() : ModulePass(ID) {
    initializeXformCbitStoresPass(*PassRegistry::getPassRegistry());
  }
...
static const char LS_NAME[] = "xform-cbit-stores";
static const char ls_name[] = "Transform Cbit Stores into a dummy function to avoid deadcode elim";
INITIALIZE_PASS_BEGIN(XformCbitStores, LS_NAME, ls_name, false, false)
INITIALIZE_PASS_END(XformCbitStores, LS_NAME, ls_name, false, false)
