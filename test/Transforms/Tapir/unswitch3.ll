...
; CHECK-NOT: %tobool = icmp eq i32 %x, 0
; CHECK: detach within %syncreg, label %det.achd, label %det.cont
  detach within %syncreg, label %det.achd, label %det.cont

det.achd:                                         ; preds = %entry
; CHECK: %goo = tail call i32 (...) @d() #3
; CHECK-NEXT: %tobool = icmp eq i32 %x, %goo
  %goo = tail call i32 (...) @d() #3
  %tobool = icmp eq i32 %x, %goo
  br i1 %tobool, label %if.else, label %if.then
; CHECK-NOT: detach within %syncreg, label %0, label %det.cont

if.then:                                          ; preds = %det.achd
  tail call void (...) @a() #3
  reattach within %syncreg, label %det.cont

; CHECK-NOT: detach within %syncreg, label %1, label %det.cont
